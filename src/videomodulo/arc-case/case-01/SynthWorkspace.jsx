...
import Mult2HPModule from './modules/Mult2HPModule'
...
const INITIAL = {
  ...
  mon1: { inputAExpr: '', inputBExpr: '', scopeExpr: '', enabled: true },
}
...
export default function SynthWorkspace() {
  ...
  const handleAddModule = useCallback((mod) => {
    console.log('Add module:', mod.type, mod.hp + 'hp')
  }, [])
  ...
  return (
    <PatchRoutingProvider>
      <div className="min-h-screen bg-surface-primary text-fg-96 flex items-end justify-center" style={{ userSelect: 'none', WebkitUserSelect: 'none' }}>
        <div className="relative flex" style={{ width: '100%', maxWidth: '1400px', marginBottom: '96px' }}>
          ...
          <div ref={rackRef} className="relative flex-1" style={{ display: 'flex', flexDirection: 'column', backgroundColor: 'rgba(20,20,20,0.95)' }}>
            ...
            <RackRow height="1u">
              <div style={HP(8)}><GateModule id="gate1" label="GATE" {...m('gate1')} /></div>
              <div style={HP(8)}><LogicModule id="logic1" label="LOGIC" {...m('logic1')} /></div>
              <div style={HP(6)}><MultiplesModule id="mult1" label="MULT" {...m('mult1')} /></div>
              <div style={HP(6)}><RandomSHModule id="sh1" label="S&H" {...m('sh1')} /></div>
              <div style={HP(16)}><MixerModule id="mix1" label="MIX" {...m('mix1')} /></div>
              <div style={HP(2)}><Mult2HPModule id="pm1" busRef={busRef} /></div>
              <div style={HP(58)}>{/* empty */}</div>
            </RackRow>

            <RackRow height="3u">
              <div style={HP(8)}><ClockModule id="clk1" label="CLK" {...m('clk1')} /></div>
              <div style={HP(2)}><Mult2HPModule id="pm2" busRef={busRef} /></div>
              <div style={HP(12)}><LFOModule id="lfo1" label="LFO" {...m('lfo1')} /></div>
              <div style={HP(12)}><LFOModule id="lfo2" label="LFO" {...m('lfo2')} /></div>
              <div style={HP(2)}><Mult2HPModule id="pm3" busRef={busRef} /></div>
              <div style={HP(16)}><SequencerModule id="seq1" label="SEQ" {...m('seq1')} /></div>
              <div style={HP(12)}><EnvelopeModule id="env1" label="ENV" {...m('env1')} /></div>
              <div style={HP(2)}><Mult2HPModule id="pm4" busRef={busRef} /></div>
              <div style={HP(16)}><MathsModule id="math1" label="MATHS" {...m('math1')} /></div>
              <div style={HP(22)}>{/* empty */}</div>
            </RackRow>

            <RackRow height="3u">
              <div style={HP(12)}><GeneratorModule id="gen1" label="GEN" {...m('gen1')} /></div>
              <div style={HP(12)}><GeneratorModule id="gen2" label="GEN" {...m('gen2')} /></div>
              <div style={HP(16)}><DitherModule id="dither1" label="DITHER" {...m('dither1')} /></div>
              <div style={HP(16)}><Geometry3DModule id="geo1" label="GEO" {...m('geo1')} /></div>
              <div style={HP(16)}><MonitorModule id="mon1" label="MON" {...m('mon1')} /></div>
              <div style={HP(32)}>{/* empty */}</div>
            </RackRow>
          </div>
          <div style={{ width: '6px', flexShrink: 0, backgroundColor: 'rgba(60,60,60,0.5)', borderRadius: '0 2px 2px 0' }} />
        </div>
      </div>
      <ModuleShelf isOpen={shelfOpen} onToggle={() => setShelfOpen(v => !v)} onAddModule={handleAddModule} />
    </PatchRoutingProvider>
  )
}
